...
csv_out = strcat(exp_dir, 'apoeint_values.csv');
to_csv_mat = [squeeze(slm_l_add.coef(8,:,:)), squeeze(slm_l_add.coef(9,:,:))];
csvwrite(csv_out, to_csv_mat);

% Call the function from python to create the volume
vtk_out = strcat(exp_dir,'apoeint_values.vtk');
...
csv_out = strcat(exp_dir, 'apoeint_values.csv');
to_csv_mat = [squeeze(slm_r.coef(8,:,:)), squeeze(slm_r.coef(9,:,:))];
csvwrite(csv_out, to_csv_mat);
