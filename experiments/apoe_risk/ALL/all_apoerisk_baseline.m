...
model_l = 1 + Age + Gender + Site + Yed + APOE_OR + DX;
model_r = 1 + Age + Gender + Site + Yed + APOE_OR + DX;

slm_l = SurfStatLinMod(t_l, model_l, avg_l);
slm_r = SurfStatLinMod(t_r, model_r, avg_r);

% ADDITIVE
model_r_add = 1 + Age + Gender + Site + Yed + APOE_OR + DX_int;
model_l_add = 1 + Age + Gender + Site + Yed + APOE_OR + DX_int;
...
save_file_add = {'apoe_or_int', '-apoe_or_int', 'AD_MCI_CN', '-AD_MCI_CN'};
